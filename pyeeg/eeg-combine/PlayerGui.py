...
class PlayerGui:
    ...
    def __init__(self,r,desktop):
        ...
        OptionMenu(self.r,self.e_frequency, command = self.cbEffFrequencySet, \
                   *self.e_freq_options ).grid(row=4,column=1,sticky=W+E)
    ...
    def cbGoClicked(self):
        ...
        if self.eeg.is_loaded == False:
            tkMessageBox.showerror(_("Error"),_("EEG-file is not set."))
            self.go_state.set(0)
            return
        ...
        if (self.player.is_playing == True):
            ...
            self.player.play()
            self.drawFantasticBlueTriangle()
    ...
    def cbPlayerIterate(self, size):
        if (size == 0):
            self.cbGoClicked()
        current = self.eeg.current
        self.scale.set(current)
        if (self.eeg.current == self.eeg.n_blocks):
            self.eeg.current = 1
            self.scale.set(self.eeg.current)
        

    def drawFantasticBlueTriangle(self):
        if self.player.is_playing == True:
            self.desktop.drawPlayer()
            self.r.after(100,self.drawFantasticBlueTriangle)            
        else:    
            self.desktop.restore()
